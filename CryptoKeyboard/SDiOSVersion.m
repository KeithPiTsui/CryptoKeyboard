...
+ (DeviceSize)resolutionSize
{
    ...
    if (screenHeight == 480) {
        ...
        if ([[self deviceName] containsString: @"mini"]) {
...
+ (NSString*) systemName {
    ...
    NSOperatingSystemVersion versions = [[NSProcessInfo processInfo] operatingSystemVersion];

    return [NSString stringWithFormat:@"iOS %li.%li.%li",
            (long)versions.majorVersion, (long)versions.minorVersion, (long)versions.patchVersion];
    
}

+ (NSString*) deviceVersionName {

    return DeviceVersionNames[[SDiOSVersion deviceVersion]];
}
